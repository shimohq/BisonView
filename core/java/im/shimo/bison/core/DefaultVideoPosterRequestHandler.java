...
public class DefaultVideoPosterRequestHandler {
    private static InputStream getInputStream(final BisonContentsClient contentClient)
            throws IOException {
    ...
    private BisonContentsClient mContentClient;

    public DefaultVideoPosterRequestHandler(BisonContentsClient contentClient) {
     ...
     * @return BisonWebResourceResponse which caller can get the image if the url is
     * the default video poster URL, otherwise null is returned.
     */
    public BisonWebResourceResponse shouldInterceptRequest(final String url) {
        if (!mDefaultVideoPosterURL.equals(url)) return null;

        try {
            return new BisonWebResourceResponse("image/png", null, getInputStream(mContentClient));
        } catch (IOException e) {
            Log.e(TAG, null, e);
            return null;
        }
