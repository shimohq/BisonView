...
#include "bison/core/browser/bison_ssl_host_state_delegate.h"
...
BisonSSLHostStateDelegate::BisonSSLHostStateDelegate() {
}

BisonSSLHostStateDelegate::~BisonSSLHostStateDelegate() {
}

void BisonSSLHostStateDelegate::HostRanInsecureContent(
...
bool BisonSSLHostStateDelegate::DidHostRunInsecureContent(
    const std::string& host,
    int child_id,
    InsecureContentType content_type) {
  // Intentional no-op for Android WebView.
...
void BisonSSLHostStateDelegate::RevokeUserAllowExceptions(
    const std::string& host) {
