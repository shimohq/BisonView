...
#include "bison/core/browser/metrics/bison_metrics_service_client.h"
...
class TestClient : public BisonMetricsServiceClient {
 public:
...
class BisonMetricsServiceClientTest : public testing::Test {
 public:
  BisonMetricsServiceClientTest() : task_runner_(new base::TestSimpleTaskRunner) {
    // Required by MetricsService.
    base::SetRecordActionTaskRunner(task_runner_);
  }

 protected:
  ~BisonMetricsServiceClientTest() override {}

 private:
  base::test::TaskEnvironment task_environment_;
  scoped_refptr<base::TestSimpleTaskRunner> task_runner_;

  DISALLOW_COPY_AND_ASSIGN(BisonMetricsServiceClientTest);
};

TEST_F(BisonMetricsServiceClientTest, TestSetConsentTrueBeforeInit) {
...
TEST_F(BisonMetricsServiceClientTest, TestSetConsentFalseBeforeInit) {
  auto prefs = CreateTestPrefs();
  auto client = std::make_unique<TestClient>();
...
TEST_F(BisonMetricsServiceClientTest, TestSetConsentTrueAfterInit) {
  auto prefs = CreateTestPrefs();
...
TEST_F(BisonMetricsServiceClientTest, TestSetConsentFalseAfterInit) {
  auto prefs = CreateTestPrefs();
  auto client = CreateAndInitTestClient(prefs.get());
  client->SetHaveMetricsConsent(false, false);
  ASSERT_FALSE(client->IsRecordingActive());
...
TEST_F(BisonMetricsServiceClientTest, TestKeepExistingClientId) {
  auto prefs = CreateTestPrefs();
...
TEST_F(BisonMetricsServiceClientTest, TestSetConsentFalseClearsClientId) {
  auto prefs = CreateTestPrefs();
  prefs->SetString(metrics::prefs::kMetricsClientID, kTestClientId);
  auto client = CreateAndInitTestClient(prefs.get());
  client->SetHaveMetricsConsent(false, false);
...
TEST_F(BisonMetricsServiceClientTest, TestCanForceEnableMetrics) {
...
TEST_F(BisonMetricsServiceClientTest, TestCanForceEnableMetricsIfAlreadyEnabled) {
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
      metrics::switches::kForceEnableMetricsReporting);
...
TEST_F(BisonMetricsServiceClientTest, TestCannotForceEnableMetricsIfAppOptsOut) {
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
      metrics::switches::kForceEnableMetricsReporting);
...
TEST_F(BisonMetricsServiceClientTest, TestPreferPersistedInstallDate) {
...
// TODO(https://crbug.com/1012025): remove this when the kInstallDate pref has
// been persisted for one or two milestones.
TEST_F(BisonMetricsServiceClientTest, TestGetInstallDateFromJavaIfMissing) {
  base::HistogramTester histogram_tester;
  auto prefs = CreateTestPrefs();
  auto client = CreateAndInitTestClient(prefs.get());
