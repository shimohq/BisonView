...
#include "bison/core/browser_jni_headers/BisonSettings_jni.h"
...
BisonSettings* BisonSettings::FromWebContents(content::WebContents* web_contents) {
...
void BisonSettings::UpdateAllowFileAccessLocked(JNIEnv* env,
                                             const JavaParamRef<jobject>& obj) {
  if (!web_contents())
    return;
...
void BisonSettings::PopulateWebPreferencesLocked(JNIEnv* env,
                                              const JavaParamRef<jobject>& obj,
                                              jlong web_prefs_ptr) {
  BisonRenderViewHostExt* render_view_host_ext = GetBisonRenderViewHostExt();
  if (!render_view_host_ext)
  ...
  javascript_can_open_windows_automatically_ =
      Java_BisonSettings_getJavaScriptCanOpenWindowsAutomaticallyLocked(env, obj);
  ...
  const bool zero_layout_height_disables_viewport_quirk =
      Java_BisonSettings_getZeroLayoutHeightDisablesViewportQuirkLocked(env, obj);
  ...
  bool support_quirks = Java_BisonSettings_getSupportLegacyQuirksLocked(env, obj);
  ...
  bool enable_supported_hardware_accelerated_features =
      Java_BisonSettings_getEnableSupportedHardwareAcceleratedFeaturesLocked(env,
                                                                          obj);
  ...
  web_prefs->do_not_update_selection_on_mutating_selection_range =
      Java_BisonSettings_getDoNotUpdateSelectionOnMutatingSelectionRange(env, obj);
