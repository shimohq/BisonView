...
#include "bison/core/browser/permission/permission_request_handler.h"

#include <utility>

#include "bison/core/browser/permission/bisonpermission_request.h"
...
class TestBisonPermissionRequestDelegate : public BisonPermissionRequestDelegate {
 public:
  TestBisonPermissionRequestDelegate(const GURL& origin,
                                  int64_t resources,
...
class TestPermissionRequestHandlerClient
 ...
 public:
  ...
  BisonPermissionRequest* request() { return request_; }
 ...
 private:
  ...
  BisonPermissionRequest* request_;
...
class TestPermissionRequestHandler : public PermissionRequestHandler {
 public:
  ...
  const std::vector<base::WeakPtr<BisonPermissionRequest>> requests() {
    return requests_;
  }
...
class PermissionRequestHandlerTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    resources_ =
        BisonPermissionRequest::VideoCapture | BisonPermissionRequest::AudioCapture;
    delegate_.reset(new TestBisonPermissionRequestDelegate(
        origin_, resources_,
...
TEST_F(PermissionRequestHandlerTest, TestMultiplePermissionRequest) {
  ...
  int64_t resources1 = BisonPermissionRequest::Geolocation;

  std::unique_ptr<BisonPermissionRequestDelegate> delegate1;
  delegate1.reset(new TestBisonPermissionRequestDelegate(
      origin1, resources1,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
...
TEST_F(PermissionRequestHandlerTest, TestPreauthorizePermission) {
  ...
  std::unique_ptr<BisonPermissionRequestDelegate> delegate;
  delegate.reset(new TestBisonPermissionRequestDelegate(
      origin(), BisonPermissionRequest::AudioCapture,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
  client()->Reset();
...
TEST_F(PermissionRequestHandlerTest, TestOriginNotPreauthorized) {
  ...
  std::unique_ptr<BisonPermissionRequestDelegate> delegate;
  int64_t requested_resources = BisonPermissionRequest::AudioCapture;
  delegate.reset(new TestBisonPermissionRequestDelegate(
...
TEST_F(PermissionRequestHandlerTest, TestResourcesNotPreauthorized) {
  ...
  int64_t requested_resources =
      BisonPermissionRequest::AudioCapture | BisonPermissionRequest::Geolocation;
  delegate.reset(new TestBisonPermissionRequestDelegate(
      origin(), requested_resources,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
...
TEST_F(PermissionRequestHandlerTest, TestPreauthorizeMultiplePermission) {
  ...
  GURL origin("http://a.google.com/a/b");
  handler()->PreauthorizePermission(origin, BisonPermissionRequest::Geolocation);
  GURL origin_hostname("http://a.google.com/");
  std::unique_ptr<BisonPermissionRequestDelegate> delegate;
  delegate.reset(new TestBisonPermissionRequestDelegate(
      origin_hostname, BisonPermissionRequest::Geolocation,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
  handler()->SendRequest(std::move(delegate));
  EXPECT_TRUE(allowed());
