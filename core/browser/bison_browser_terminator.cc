...
#include "bison/core/browser/bison_browser_terminator.h"

#include <unistd.h>
#include <memory>

#include "bison/core/browser/bison_render_process_gone_delegate.h"
#include "bison/core/browser_jni_headers/BisonBrowserProcess_jni.h"
...
void OnRenderProcessGone(
  ...
  for (auto& java_wc : java_web_contents) {
    ...
    BisonRenderProcessGoneDelegate* delegate =
        BisonRenderProcessGoneDelegate::FromWebContents(wc);
    if (!delegate)
      continue;

    switch (delegate->OnRenderProcessGone(child_process_pid, crashed)) {
      case BisonRenderProcessGoneDelegate::RenderProcessGoneResult::kException:
        // Let the exception propagate back to the message loop.
        base::MessageLoopCurrentForUI::Get()->Abort();
        return;
      case BisonRenderProcessGoneDelegate::RenderProcessGoneResult::kUnhandled:
        if (crashed) {
        ...
        NOTREACHED();
        break;
      case BisonRenderProcessGoneDelegate::RenderProcessGoneResult::kHandled:
        break;
    }
  }
  ...
  Java_BisonBrowserProcess_triggerMinidumpUploading(
      base::android::AttachCurrentThread());
}

}  // namespace

BisonBrowserTerminator::BisonBrowserTerminator() = default;

BisonBrowserTerminator::~BisonBrowserTerminator() = default;

void BisonBrowserTerminator::OnChildExit(
