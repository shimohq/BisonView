...
#include "bison/core/browser/gfx/compositor_frame_consumer.h"
#include "bison/core/browser/gfx/root_frame_sink.h"
#include "bison/core/browser/gfx/root_frame_sink_proxy.h"
...
void BrowserViewRenderer::TrimMemory() {
  ...
  TRACE_EVENT0("bison", "BrowserViewRenderer::TrimMemory");
...
bool BrowserViewRenderer::CanOnDraw() {
  if (!compositor_) {
    TRACE_EVENT_INSTANT0("bison", "EarlyOut_NoCompositor",
                         TRACE_EVENT_SCOPE_THREAD);
    return false;
  }
  if (clear_view_) {
    TRACE_EVENT_INSTANT0("bison", "EarlyOut_ClearView",
                         TRACE_EVENT_SCOPE_THREAD);
    return false;
  }
...
bool BrowserViewRenderer::OnDrawHardware() {
  DCHECK(current_compositor_frame_consumer_);
  TRACE_EVENT0("bison", "BrowserViewRenderer::OnDrawHardware");
...
sk_sp<SkPicture> BrowserViewRenderer::CapturePicture(int width,
                                                     int height) {
  TRACE_EVENT0("bison", "BrowserViewRenderer::CapturePicture");
...
void BrowserViewRenderer::OnDetachedFromWindow() {
  TRACE_EVENT0("bison", "BrowserViewRenderer::OnDetachedFromWindow");
  attached_to_window_ = false;
  ReleaseHardware();
}
...
void BrowserViewRenderer::OnComputeScroll(base::TimeTicks animation_time) {
  if (!compositor_)
    return;
  TRACE_EVENT0("bison", "BrowserViewRenderer::OnComputeScroll");
...
void BrowserViewRenderer::ScrollTo(const gfx::Vector2d& scroll_offset) {
  ...
  TRACE_EVENT_INSTANT2("bison", "BrowserViewRenderer::ScrollTo",
...
void BrowserViewRenderer::UpdateRootLayerState(
  ...
  TRACE_EVENT_INSTANT1(
      "bison", "BrowserViewRenderer::UpdateRootLayerState",
...
void BrowserViewRenderer::PostInvalidate(
    content::SynchronousCompositor* compositor) {
  TRACE_EVENT_INSTANT0("bison", "BrowserViewRenderer::PostInvalidate",
                       TRACE_EVENT_SCOPE_THREAD);
  if (compositor != compositor_)
    return;
