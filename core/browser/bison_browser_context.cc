...
#include "bison/core/browser/bison_download_manager_delegate.h"
#include "bison/core/browser/bison_form_database_service.h"
...
void MigrateProfileData(base::FilePath cache_path,
  ...
  old_cache_path = old_cache_path.DirName().Append(
      FILE_PATH_LITERAL("org.chromium.bison"));
...
void BisonBrowserContext::MigrateLocalStatePrefs() {
  PrefService* local_state = BisonBrowserProcess::GetInstance()->local_state();
...
content::ResourceContext* BisonBrowserContext::GetResourceContext() {
...
content::BrowserPluginGuestManager* BisonBrowserContext::GetGuestManager() {
  return NULL;
}
...
content::PushMessagingService* BisonBrowserContext::GetPushMessagingService() {
...
content::SSLHostStateDelegate* BisonBrowserContext::GetSSLHostStateDelegate() {
...
BisonBrowserContext::GetBackgroundFetchDelegate() {
  return nullptr;
}
...
BisonBrowserContext::GetJavaBrowserContext() {
  if (!obj_) {
    ...
    obj_ = Java_BisonBrowserContext_create(env, reinterpret_cast<intptr_t>(this),
