...
BisonSafeBrowsingWhitelistManager::BisonSafeBrowsingWhitelistManager(
...
void BisonSafeBrowsingWhitelistManager::SetWhitelist(
...
void BisonSafeBrowsingWhitelistManager::BuildWhitelist(
  ...
  if (success) {
    // use base::Unretained as BisonSafeBrowsingWhitelistManager is a singleton and
    // not cleaned.
    io_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&BisonSafeBrowsingWhitelistManager::SetWhitelist,
                       base::Unretained(this), std::move(whitelist)));
  }
}

void BisonSafeBrowsingWhitelistManager::SetWhitelistOnUIThread(
  ...
  background_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&BisonSafeBrowsingWhitelistManager::BuildWhitelist,
                                base::Unretained(this), std::move(rules),
                                std::move(callback)));
}

bool BisonSafeBrowsingWhitelistManager::IsURLWhitelisted(const GURL& url) const {
  DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
  if (!url.has_host()) {
    return false;
  }
