...
import im.shimo.bison.common.CommandLineUtil;
import im.shimo.bison.common.PlatformServiceBridge;
import im.shimo.bison.common.ServiceNames;
import im.shimo.bison.metrics.BisonMetricsServiceClient;
import im.shimo.bison.policy.BisonPolicyProvider;
import im.shimo.bison.services.ICrashReceiverService;

/**
 * Wrapper for the steps needed to initialize the java and native sides of webview chromium.
 */
@JNINamespace("bison")
public final class BisonBrowserProcess {
    private static final String TAG = "BisonBrowserProcess";
    ...
    public static void start() {
        try (ScopedSysTraceEvent e1 = ScopedSysTraceEvent.scoped("BisonBrowserProcess.start")) {
    ...
    private static void tryObtainingDataDirLock(final Context appContext) {
        try (ScopedSysTraceEvent e1 =
                        ScopedSysTraceEvent.scoped("BisonBrowserProcess.tryObtainingDataDirLock")) {
    ...
    public static void handleMinidumpsAndSetMetricsConsent(final boolean updateMetricsConsent) {
        try (ScopedSysTraceEvent e1 = ScopedSysTraceEvent.scoped(
                     "BisonBrowserProcess.handleMinidumpsAndSetMetricsConsent")) {
            ...
            PlatformServiceBridge.getInstance().queryMetricsSetting(enabled -> {
                ...
                if (updateMetricsConsent) {
                    BisonMetricsServiceClient.setConsentSetting(
