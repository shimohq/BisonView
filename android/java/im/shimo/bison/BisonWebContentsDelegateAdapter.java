 ...
 */
class BisonWebContentsDelegateAdapter extends BisonWebContentsDelegate {
    private static final String TAG = "BisonWebContentsDelegateAdapter";

    private final BisonContents mBisonContents;
    private final BisonContentsClient mContentsClient;
    private final BisonSettings mAwSettings;
    private final Context mContext;
    private View mContainerView;
    private FrameLayout mCustomView;

    public BisonWebContentsDelegateAdapter(BisonContents bisonContents, BisonContentsClient contentsClient,
                                           BisonSettings settings, Context context, View containerView) {
    ...
    public boolean addMessageToConsole(int level, String message, int lineNumber,
            String sourceId) {
        @BisonConsoleMessage.MessageLevel
        int messageLevel = BisonConsoleMessage.MESSAGE_LEVEL_DEBUG;
        switch(level) {
            case LOG_LEVEL_TIP:
                messageLevel = BisonConsoleMessage.MESSAGE_LEVEL_TIP;
                break;
            case LOG_LEVEL_LOG:
                messageLevel = BisonConsoleMessage.MESSAGE_LEVEL_LOG;
                break;
            case LOG_LEVEL_WARNING:
                messageLevel = BisonConsoleMessage.MESSAGE_LEVEL_WARNING;
                break;
            case LOG_LEVEL_ERROR:
                messageLevel = BisonConsoleMessage.MESSAGE_LEVEL_ERROR;
                break;
            default:
        ...
        boolean result = mContentsClient.onConsoleMessage(
                new BisonConsoleMessage(message, sourceId, lineNumber, messageLevel));
        return result;
    }
    ...
    public void showRepostFormWarningDialog() {
        ...
        final Handler handler = new Handler(ThreadUtils.getUiThreadLooper()) {
            ...
            public void handleMessage(Message msg) {
                if (mBisonContents.getNavigationController() == null) return;

                switch(msg.what) {
                    case msgContinuePendingReload: {
                        mBisonContents.getNavigationController().continuePendingReload();
                        break;
                    }
                    case msgCancelPendingReload: {
                        mBisonContents.getNavigationController().cancelPendingReload();
                        break;
                    }
                    default:
    ...
    public void navigationStateChanged(int flags) {
        if ((flags & InvalidateTypes.URL) != 0
                && mBisonContents.isPopupWindow()
                && mBisonContents.hasAccessedInitialDocument()) {
            // Hint the client to show the last committed url, as it may be unsafe to show
            // the pending entry.
            String url = mBisonContents.getLastCommittedUrl();
    ...
    private void enterFullscreen() {
        if (mBisonContents.isFullScreen()) {
            return;
        }
        View fullscreenView = mBisonContents.enterFullScreen();
        if (fullscreenView == null) {
            return;
        }
        BisonContentsClient.CustomViewCallback cb = () -> {
            if (mCustomView != null) {
