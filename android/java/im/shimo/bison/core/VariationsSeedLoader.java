...
import im.shimo.bison.core.common.variations.VariationsUtils;
import im.shimo.bison.core.services.IVariationsSeedServer;
...
public class VariationsSeedLoader {
    ...
    private class SeedServerConnection implements ServiceConnection {
        ...
        public void start() {
            try {
                ...
                Log.e(TAG, "WebView provider \"" + BisonBrowserProcess.getWebViewPackageName() +
                        "\" not found!");
            }
        }
    ...
    protected Intent getServerIntent() throws NameNotFoundException {
        ...
        intent.setClassName(
                BisonBrowserProcess.getWebViewPackageName(), ServiceNames.VARIATIONS_SEED_SERVER);
        return intent;
    }
    ...
    public void finishVariationsInit() {
        ...
        if (seed != null) {
            BisonVariationsSeedBridge.setSeed(seed);
            BisonVariationsSeedBridge.setLoadedSeedFresh(isLoadedSeedFresh());
        }
    }
}
