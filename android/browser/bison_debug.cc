...
#include "bison/android/browser_jni_headers/BisonDebug_jni.h"
...
class BisonDebugCrashReporterClient
    : public ::crash_reporter::CrashReporterClient {
 public:
  BisonDebugCrashReporterClient() = default;
  ~BisonDebugCrashReporterClient() override = default;

  void GetProductNameAndVersion(std::string* product_name,
                                std::string* version,
                                std::string* channel) override {
    // TODO jiang rename to bison                              
  ...
  DISALLOW_COPY_AND_ASSIGN(BisonDebugCrashReporterClient);
};
...
static jboolean JNI_BisonDebug_DumpWithoutCrashing(
    JNIEnv* env,
  ...
  BisonDebugCrashReporterClient client;
...
static void JNI_BisonDebug_InitCrashKeysForWebViewTesting(JNIEnv* env) {
  crash_keys::InitCrashKeysForWebViewTesting();
}

static void JNI_BisonDebug_SetWhiteListedKeyForTesting(JNIEnv* env) {
  static ::crash_reporter::CrashKeyString<32> crash_key(
      "AW_WHITELISTED_DEBUG_KEY");
  crash_key.Set("AW_DEBUG_VALUE");
}

static void JNI_BisonDebug_SetNonWhiteListedKeyForTesting(JNIEnv* env) {
  static ::crash_reporter::CrashKeyString<32> crash_key(
