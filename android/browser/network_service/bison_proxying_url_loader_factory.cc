...
#include "bison/android/browser/network_service/bison_proxying_url_loader_factory.h"
...
#include "bison/android/browser/network_service/android_stream_reader_url_loader.h"
...
#include "bison/android/browser/renderer_host/auto_login_parser.h"
...
class InterceptResponseDelegate
 ...
 public:
  ...
  std::unique_ptr<bison::InputStream> OpenInputStream(
      JNIEnv* env) override {
...
BisonContentsClientBridge* GetBisonContentsClientBridgeFromID(int process_id,
                                                        int render_frame_id) {
...
void OnReceivedHttpErrorOnUiThread(
  ...
  auto* client = GetBisonContentsClientBridgeFromID(process_id, render_frame_id);
  if (!client) {
...
BisonProxyingURLLoaderFactory::BisonProxyingURLLoaderFactory(
  ...
  proxy_receivers_.set_disconnect_handler(
      base::BindRepeating(&BisonProxyingURLLoaderFactory::OnProxyBindingError,
                          base::Unretained(this)));
}
...
void BisonProxyingURLLoaderFactory::OnTargetFactoryError() {
  delete this;
}

void BisonProxyingURLLoaderFactory::OnProxyBindingError() {
  if (proxy_receivers_.empty())
    delete this;
}

void BisonProxyingURLLoaderFactory::Clone(
    mojo::PendingReceiver<network::mojom::URLLoaderFactory> loader_receiver) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::IO);
  proxy_receivers_.Add(this, std::move(loader_receiver));
}
