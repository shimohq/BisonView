...
int GetCacheModeForClient(BisonContentsIoThreadClient* client) {
  DCHECK(client);
  BisonContentsIoThreadClient::CacheMode cache_mode = client->GetCacheMode();
  switch (cache_mode) {
    case BisonContentsIoThreadClient::LOAD_CACHE_ELSE_NETWORK:
    ...
    case BisonContentsIoThreadClient::LOAD_NO_CACHE:
      // Always load from the network, don't use the cache.
      return net::LOAD_BYPASS_CACHE;
    case BisonContentsIoThreadClient::LOAD_CACHE_ONLY:
