...
#include "bison/android/browser/bison_permission_manager.h"

#include <list>
#include <memory>

#include "bison/android/browser/bison_browser_permission_request_delegate.h"
#include "base/bind.h"
...
class BisonBrowserPermissionRequestDelegateForTesting final
...
class BisonPermissionManagerForTesting : public BisonPermissionManager {
 public:
  ~BisonPermissionManagerForTesting() override {
 ...
 private:
  BisonBrowserPermissionRequestDelegateForTesting* delegate() {
    if (!delegate_)
      delegate_.reset(new BisonBrowserPermissionRequestDelegateForTesting);
  ...
  BisonBrowserPermissionRequestDelegate* GetDelegate(
  ...
  std::unique_ptr<BisonBrowserPermissionRequestDelegateForTesting> delegate_;
};

class BisonPermissionManagerTest : public testing::Test {
 public:
  BisonPermissionManagerTest()
 ...
 protected:
  void SetUp() override { manager.reset(new BisonPermissionManagerForTesting); }
  ...
  std::unique_ptr<BisonPermissionManagerForTesting> manager;

  // Use nullptr for testing. BisonPermissionManagerForTesting override all methods
...
TEST_F(BisonPermissionManagerTest, MIDIPermissionIsGrantedSynchronously) {
  int request_id = manager->RequestPermission(
      PermissionType::MIDI, render_frame_host, GURL(kRequestingOrigin1), true,
      base::Bind(&BisonPermissionManagerTest::PermissionRequestResponse,
...
// synchronously.
TEST_F(BisonPermissionManagerTest, SinglePermissionRequestIsGrantedSynchronously) {
  ...
  int request_id = manager->RequestPermission(
      ...
      true, base::Bind(&BisonPermissionManagerTest::PermissionRequestResponse,
                       base::Unretained(this), 0));
  EXPECT_EQ(kNoPendingOperation, request_id);
  ASSERT_EQ(1u, resolved_permission_status.size());
  EXPECT_EQ(PermissionStatus::GRANTED, resolved_permission_status[0]);
...
// Test the case a delegate is called, and it resolves the permission
// asynchronously.
TEST_F(BisonPermissionManagerTest,
...
TEST_F(BisonPermissionManagerTest, ManagerIsDeletedWhileDelegateProcesses) {
  int request_id = manager->RequestPermission(
      PermissionType::GEOLOCATION, render_frame_host, GURL(kRequestingOrigin1),
      true, base::Bind(&BisonPermissionManagerTest::PermissionRequestResponse,
                       base::Unretained(this), 0));
...
TEST_F(BisonPermissionManagerTest,
  ...
  int request_id = manager->RequestPermissions(
      ...
      true, base::Bind(&BisonPermissionManagerTest::PermissionsRequestResponse,
                       base::Unretained(this), 0));
  EXPECT_EQ(kNoPendingOperation, request_id);
...
TEST_F(BisonPermissionManagerTest, ComplicatedRequestScenario1) {
...
//     resolved.
TEST_F(BisonPermissionManagerTest, ComplicatedRequestScenario2) {
  // In the first case, the permission A is a type that does not call an
  // internal delegate method.
  std::vector<PermissionType> permissions_1 = {PermissionType::MIDI,
                                               PermissionType::MIDI_SYSEX};
...
TEST_F(BisonPermissionManagerTest, ComplicatedRequestScenario3) {
  // In the first case, the permission A is a type that does not call an
  // internal delegate method.
  std::vector<PermissionType> permissions_1 = {PermissionType::MIDI,
                                               PermissionType::MIDI_SYSEX};
...
//     resolved.
TEST_F(BisonPermissionManagerTest, ComplicatedRequestScenario4) {
  // In the first case, the permission A is a type that does not call an
  // internal delegate method.
  std::vector<PermissionType> permissions_1 = {PermissionType::MIDI,
                                               PermissionType::MIDI_SYSEX};
