...
class BisonPermissionManager::PendingRequest {
 public:
...
int BisonPermissionManager::RequestPermissions(
  ...
  BisonBrowserPermissionRequestDelegate* delegate =
...
// static
void BisonPermissionManager::OnRequestResponse(
    const base::WeakPtr<BisonPermissionManager>& manager,
    int request_id,
    PermissionType permission,
    bool allowed) {
...
void BisonPermissionManager::ResetPermission(PermissionType permission,
                                          const GURL& requesting_origin,
                                          const GURL& embedding_origin) {
  result_cache_->ClearResult(permission, requesting_origin, embedding_origin);
}

PermissionStatus BisonPermissionManager::GetPermissionStatus(
    PermissionType permission,
    const GURL& requesting_origin,
    const GURL& embedding_origin) {
...
PermissionStatus BisonPermissionManager::GetPermissionStatusForFrame(
    PermissionType permission,
    content::RenderFrameHost* render_frame_host,
    const GURL& requesting_origin) {
...
void BisonPermissionManager::CancelPermissionRequest(int request_id) {
  ...
  BisonBrowserPermissionRequestDelegate* delegate = GetDelegate(
...
void BisonPermissionManager::CancelPermissionRequests() {
...
int BisonPermissionManager::GetRenderProcessID(
    content::RenderFrameHost* render_frame_host) {
  return render_frame_host->GetProcess()->GetID();
}

int BisonPermissionManager::GetRenderFrameID(
    content::RenderFrameHost* render_frame_host) {
  return render_frame_host->GetRoutingID();
}

GURL BisonPermissionManager::LastCommittedOrigin(
    content::RenderFrameHost* render_frame_host) {
  return content::WebContents::FromRenderFrameHost(render_frame_host)
      ->GetLastCommittedURL().GetOrigin();
}

BisonBrowserPermissionRequestDelegate* BisonPermissionManager::GetDelegate(
    int render_process_id, int render_frame_id) {
  return BisonBrowserPermissionRequestDelegate::FromID(render_process_id,
                                                    render_frame_id);
}
