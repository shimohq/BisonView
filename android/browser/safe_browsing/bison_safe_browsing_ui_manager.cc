...
#include "bison/android/browser/safe_browsing/bison_safe_browsing_blocking_page.h"
...
BisonSafeBrowsingUIManager::BisonSafeBrowsingUIManager() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);

  // TODO(timvolodine): verify this is what we want regarding the directory.
  base::FilePath user_data_dir;
  bool result = base::PathService::Get(bison::DIR_SAFE_BROWSING,
                                       &user_data_dir);
  DCHECK(result);
...
void BisonSafeBrowsingUIManager::DisplayBlockingPage(
...
void BisonSafeBrowsingUIManager::ShowBlockingPageForResource(
    const UnsafeResource& resource) {
  BisonSafeBrowsingBlockingPage::ShowBlockingPage(this, resource);
}

scoped_refptr<network::SharedURLLoaderFactory>
BisonSafeBrowsingUIManager::GetURLLoaderFactoryOnIOThread() {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  if (!shared_url_loader_factory_on_io_) {
    base::PostTask(
        FROM_HERE, {BrowserThread::UI},
        base::BindOnce(&BisonSafeBrowsingUIManager::CreateURLLoaderFactoryForIO,
...
int BisonSafeBrowsingUIManager::GetErrorUiType(
...
void BisonSafeBrowsingUIManager::SendSerializedThreatDetails(
...
void BisonSafeBrowsingUIManager::CreateURLLoaderFactoryForIO(
