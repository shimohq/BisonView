...
#include "bison_crash_reporter_client.h"
...
#include "bison/android/common_jni_headers/BisonCrashReporterClient_jni.h"
...
class BisonCrashReporterClient : public crash_reporter::CrashReporterClient {
 public:
  BisonCrashReporterClient() {}
  ...
  bool JavaExceptionFilter(
    ...
    return Java_BisonCrashReporterClient_stackTraceContainsWebViewCode(
        AttachCurrentThread(), java_exception);
  }

 private:
  DISALLOW_COPY_AND_ASSIGN(BisonCrashReporterClient);
};

base::LazyInstance<BisonCrashReporterClient>::Leaky g_crash_reporter_client =
...
void EnableCrashReporter(const std::string& process_type) {
  ...
  base::android::SetJavaExceptionFilter(base::BindRepeating(
      &BisonCrashReporterClient::JavaExceptionFilter, base::Unretained(client)));
  g_enabled = true;
}
