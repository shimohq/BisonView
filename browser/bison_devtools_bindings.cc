...
std::vector<BisonDevToolsBindings*>* GetBisonDevToolsBindingsInstances() {
  static base::NoDestructor<std::vector<BisonDevToolsBindings*>> instance;
  return instance.get();
}
...
class BisonDevToolsBindings::NetworkResourceLoader
    : public network::SimpleURLLoaderStreamConsumer {
 public:
  NetworkResourceLoader(int stream_id,
                        int request_id,
                        BisonDevToolsBindings* bindings,
 ...
 private:
  ...
  BisonDevToolsBindings* const bindings_;
...
void BisonDevToolsBindings::InspectElementAt(int x, int y) {
  if (agent_host_) {
...
BisonDevToolsBindings::BisonDevToolsBindings(WebContents* devtools_contents,
  ...
  auto* bindings = GetBisonDevToolsBindingsInstances();
  DCHECK(!base::Contains(*bindings, this));
  bindings->push_back(this);
}

BisonDevToolsBindings::~BisonDevToolsBindings() {
  if (agent_host_)
...
// static
std::vector<BisonDevToolsBindings*>
BisonDevToolsBindings::GetInstancesForWebContents(WebContents* web_contents) {
  auto* bindings = GetBisonDevToolsBindingsInstances();
  std::vector<BisonDevToolsBindings*> result;
  std::copy_if(bindings->begin(), bindings->end(), std::back_inserter(result),
               [web_contents](BisonDevToolsBindings* binding) {
                 return binding->inspected_contents() == web_contents;
               });
  return result;
}

void BisonDevToolsBindings::ReadyToCommitNavigation(
  ...
  if (navigation_handle->IsInMainFrame()) {
    frontend_host_ = DevToolsFrontendHost::Create(
        frame, base::BindRepeating(
                   &BisonDevToolsBindings::HandleMessageFromDevToolsFrontend,
...
void BisonDevToolsBindings::AttachInternal() {
  if (agent_host_)
    agent_host_->DetachClient(this);
...
void BisonDevToolsBindings::Attach() {
  AttachInternal();
}

void BisonDevToolsBindings::UpdateInspectedWebContents(
...
void BisonDevToolsBindings::WebContentsDestroyed() {
  if (agent_host_) {
    agent_host_->DetachClient(this);
    agent_host_ = nullptr;
  }
...
void BisonDevToolsBindings::HandleMessageFromDevToolsFrontend(
...
void BisonDevToolsBindings::DispatchProtocolMessage(
...
void BisonDevToolsBindings::CallClientFunction(const std::string& function_name,
...
void BisonDevToolsBindings::SendMessageAck(int request_id,
                                           const base::Value* arg) {
  base::Value id_value(request_id);
  CallClientFunction("DevToolsAPI.embedderMessageAck", &id_value, arg, nullptr);
}

void BisonDevToolsBindings::AgentHostClosed(DevToolsAgentHost* agent_host) {
  agent_host_ = nullptr;
  if (delegate_)
    delegate_->Close();
}
