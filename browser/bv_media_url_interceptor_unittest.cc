...
class BvMediaUrlInterceptorTest : public Test {
 public:
  BvMediaUrlInterceptorTest()
      : fd_(UNSET_VALUE),
        offset_(UNSET_VALUE),
        size_(UNSET_VALUE),
        url_interceptor_(new BvMediaUrlInterceptor()) {}

 protected:
  int fd_;
  int64_t offset_;
  int64_t size_;
  std::unique_ptr<BvMediaUrlInterceptor> url_interceptor_;
};

}  // namespace

TEST_F(BvMediaUrlInterceptorTest, TestInterceptValidAssetUrl) {
  // This asset file exists in the bison_view_unittests-debug.apk.
  // See gyp rule bison_view_unittests_apk.
...
// TODO(crbug/784572)
TEST_F(BvMediaUrlInterceptorTest, DISABLED_TestInterceptInvalidAssetUrl) {
  // This asset file does not exist in the bison_view_unittests-debug.apk.
  // See gyp rule bison_view_unittests_apk.
...
TEST_F(BvMediaUrlInterceptorTest, TestInterceptNonAssetUrl) {
