...
class TestBvPermissionRequestDelegate : public BvPermissionRequestDelegate {
 public:
  TestBvPermissionRequestDelegate(const GURL& origin,
                                  int64_t resources,
...
class TestPermissionRequestHandlerClient
 ...
 public:
  ...
  BvPermissionRequest* request() { return request_; }
...
class TestPermissionRequestHandler : public PermissionRequestHandler {
 public:
  ...
  const std::vector<base::WeakPtr<BvPermissionRequest>> requests() {
    return requests_;
  }
...
class PermissionRequestHandlerTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    resources_ =
        BvPermissionRequest::VideoCapture | BvPermissionRequest::AudioCapture;
    delegate_.reset(new TestBvPermissionRequestDelegate(
        origin_, resources_,
...
TEST_F(PermissionRequestHandlerTest, TestMultiplePermissionRequest) {
  ...
  int64_t resources1 = BvPermissionRequest::Geolocation;

  std::unique_ptr<BvPermissionRequestDelegate> delegate1;
  delegate1.reset(new TestBvPermissionRequestDelegate(
      origin1, resources1,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
...
TEST_F(PermissionRequestHandlerTest, TestPreauthorizePermission) {
  ...
  std::unique_ptr<BvPermissionRequestDelegate> delegate;
  delegate.reset(new TestBvPermissionRequestDelegate(
      origin(), BvPermissionRequest::AudioCapture,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
  client()->Reset();
...
TEST_F(PermissionRequestHandlerTest, TestOriginNotPreauthorized) {
  ...
  std::unique_ptr<BvPermissionRequestDelegate> delegate;
  int64_t requested_resources = BvPermissionRequest::AudioCapture;
  delegate.reset(new TestBvPermissionRequestDelegate(
...
TEST_F(PermissionRequestHandlerTest, TestResourcesNotPreauthorized) {
  ...
  int64_t requested_resources =
      BvPermissionRequest::AudioCapture | BvPermissionRequest::Geolocation;
  delegate.reset(new TestBvPermissionRequestDelegate(
      origin(), requested_resources,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
...
TEST_F(PermissionRequestHandlerTest, TestPreauthorizeMultiplePermission) {
  ...
  GURL origin("http://a.google.com/a/b");
  handler()->PreauthorizePermission(origin, BvPermissionRequest::Geolocation);
  GURL origin_hostname("http://a.google.com/");
  std::unique_ptr<BvPermissionRequestDelegate> delegate;
  delegate.reset(new TestBvPermissionRequestDelegate(
      origin_hostname, BvPermissionRequest::Geolocation,
      base::BindRepeating(&PermissionRequestHandlerTest::NotifyRequestResult,
                          base::Unretained(this))));
  handler()->SendRequest(std::move(delegate));
  EXPECT_TRUE(allowed());
