...
// |BvContentsIoThreadClient| in a native |BvContentsIoThreadClient| by getting
...
std::unique_ptr<BvContentsIoThreadClient> WrapOptionalWeakRef(
  ...
  if (opt_delegate_weak_ref) {
    ...
    if (java_delegate) {
      return std::make_unique<BvContentsIoThreadClient>(java_delegate);
    }
  }
  return nullptr;
}
...
// static
std::unique_ptr<BvContentsIoThreadClient> BvContentsIoThreadClient::FromID(
...
BvContentsIoThreadClient::BvContentsIoThreadClient(const JavaRef<jobject>& obj)
    : java_object_(obj) {
  DCHECK(java_object_);
}
...
BvContentsIoThreadClient::CacheMode BvContentsIoThreadClient::GetCacheMode()
    const {
...
void RecordInterceptedScheme(bool response_is_null, const std::string& url) {
  ...
  UMA_HISTOGRAM_ENUMERATION(
      "BisonView.ShouldInterceptRequest.InterceptionType2", type);
}

std::unique_ptr<BvWebResourceInterceptResponse> NoInterceptRequest() {
  return nullptr;
}

std::unique_ptr<BvWebResourceInterceptResponse> RunShouldInterceptRequest(
  ...
  auto web_resource_intercept_response =
      std::make_unique<BvWebResourceInterceptResponse>(java_ref);
...
void BvContentsIoThreadClient::ShouldInterceptRequestAsync(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  base::OnceCallback<std::unique_ptr<BvWebResourceInterceptResponse>()>
...
bool BvContentsIoThreadClient::ShouldBlockFileUrls() const {
  ...
  JNIEnv* env = AttachCurrentThread();
  return Java_BvContentsIoThreadClient_shouldBlockFileUrls(env, java_object_);
}
